...
6. 上述基础设施失败在修复后再次出现时停止派发。后续周期只做一次短 SSH/协调器/Mongo 连通性检查，三项恢复后才允许再派发，避免用空运行持续冲击测试服。
7. 如果出现 HTTP 429、熔断、地区限制、超过 10% 游戏失败、没有有效新增或吞吐显著下降，不派发，通知用户具体运行、指标和受影响游戏。第 5 条明确允许的首次采集前基础设施失败除外。
8. 全部配额达标后不再派发；确认 `validate-data.ts`、测试服 Mongo 审计和 `finalize-data.ts --target test` 通过，再通知完成。
